...
func (db *DB) GetAllProjects() ([]*Project, error) {
	rows, err := db.sqldb.Query("SELECT id, name, fullname FROM projects ORDER BY id")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) AddProject(name string, fullname string) (uint32, error) {
	...
	stmt, err := db.sqldb.Prepare("INSERT INTO projects(name, fullname) VALUES ($1, $2) RETURNING id")
	if err != nil {
		return 0, err
	}
...
func (db *DB) UpdateProject(id uint32, newName string, newFullname string) error {
	...
	if newName != "" && newFullname != "" {
		...
		stmt, err := db.sqldb.Prepare("UPDATE projects SET name = $1 WHERE id = $2")
		if err != nil {
			return err
		}
		...
		stmt, err := db.sqldb.Prepare("UPDATE projects SET fullname = $1 WHERE id = $2")
		if err != nil {
			return err
		}
...
func (db *DB) DeleteProject(id uint32) error {
	...
	// FIXME consider whether to move out into one-time-prepared statement
	stmt, err := db.sqldb.Prepare("DELETE FROM projects WHERE id = $1")
	if err != nil {
		return err
	}
