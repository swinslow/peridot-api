...
func TestShouldGetFileHashByID(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT id, hash_s256, hash_s1 FROM file_hashes WHERE id = \$1`).
		WithArgs(3).
...
func TestShouldGetMultipleFileHashesForSliceOfIDs(t *testing.T) {
	...
	sentRows := sqlmock.NewRows([]string{"id", "hash_s256", "hash_s1"}).
		...
		AddRow(3, s256id3, s1id3)
	mock.ExpectQuery(`SELECT id, hash_s256, hash_s1 FROM file_hashes WHERE id IN (\$1) ORDER BY id`).
...
func TestShouldAddFileHash(t *testing.T) {
	...
	regexStmt := `[INSERT INTO file_hashes(hash_s256, hash_s1) VALUES (\$1, \$2) RETURNING id]`
	mock.ExpectPrepare(regexStmt)
	stmt := "INSERT INTO file_hashes"
...
func TestShouldDeleteFileHash(t *testing.T) {
	...
	regexStmt := `[DELETE FROM file_hashes WHERE id = \$1]`
	mock.ExpectPrepare(regexStmt)
	stmt := "DELETE FROM file_hashes"
