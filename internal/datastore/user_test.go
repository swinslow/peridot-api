...
func TestShouldGetAllUsers(t *testing.T) {
	...
	mock.ExpectQuery("SELECT id, email, name, access_level FROM users ORDER BY id").WillReturnRows(sentRows)
...
func TestShouldGetUserByID(t *testing.T) {
	...
	sentRows := sqlmock.NewRows([]string{"id", "email", "name", "access_level"}).
		AddRow(8103918, "janedoe@example.com", "Jane Doe", AccessAdmin)
	mock.ExpectQuery(`[SELECT id, email, name, access_level FROM users WHERE id = \$1]`).
		WithArgs(8103918).
...
func TestShouldGetUserByEmail(t *testing.T) {
	...
	sentRows := sqlmock.NewRows([]string{"id", "email", "name", "access_level"}).
		AddRow(8103918, "janedoe@example.com", "Jane Doe", AccessAdmin)
	mock.ExpectQuery(`[SELECT id, email, name, access_level FROM users WHERE email = \$1]`).
...
func TestShouldAddUser(t *testing.T) {
	...
	regexStmt := `[INSERT INTO users(id, email, name, access_level) VALUES (\$1, \$2, \$3, \$4)]`
