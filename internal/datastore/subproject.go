...
func (db *DB) GetAllSubprojects() ([]*Subproject, error) {
	rows, err := db.sqldb.Query("SELECT id, project_id, name, fullname FROM subprojects ORDER BY id")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) GetAllSubprojectsForProjectID(projectID uint32) ([]*Subproject, error) {
	rows, err := db.sqldb.Query("SELECT id, project_id, name, fullname FROM subprojects WHERE project_id = $1 ORDER BY id", projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) AddSubproject(projectID uint32, name string, fullname string) (uint32, error) {
	...
	stmt, err := db.sqldb.Prepare("INSERT INTO subprojects(project_id, name, fullname) VALUES ($1, $2, $3) RETURNING id")
	if err != nil {
		return 0, err
	}
...
func (db *DB) UpdateSubproject(id uint32, newName string, newFullname string) error {
	...
	if newName != "" && newFullname != "" {
		stmt, err := db.sqldb.Prepare("UPDATE subprojects SET name = $1, fullname = $2 WHERE id = $3")
		if err != nil {
			return err
		}
		...
		stmt, err := db.sqldb.Prepare("UPDATE subprojects SET name = $1 WHERE id = $2")
		if err != nil {
			return err
		}
		...
		stmt, err := db.sqldb.Prepare("UPDATE subprojects SET fullname = $1 WHERE id = $2")
		if err != nil {
			return err
		}
...
func (db *DB) UpdateSubprojectProjectID(id uint32, newProjectID uint32) error {
	...
	// FIXME consider whether to move out into one-time-prepared statement
	stmt, err := db.sqldb.Prepare("UPDATE subprojects SET project_id = $1 WHERE id = $2")
	if err != nil {
		return err
	}
...
func (db *DB) DeleteSubproject(id uint32) error {
	...
	// FIXME consider whether to move out into one-time-prepared statement
	stmt, err := db.sqldb.Prepare("DELETE FROM subprojects WHERE id = $1")
	if err != nil {
		return err
	}
