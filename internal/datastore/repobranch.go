...
func (db *DB) GetAllRepoBranchesForRepoID(repoID uint32) ([]*RepoBranch, error) {
	rows, err := db.sqldb.Query("SELECT repo_id, branch FROM repo_branches WHERE repo_id = $1 ORDER BY branch", repoID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) AddRepoBranch(repoID uint32, branch string) error {
	...
	stmt, err := db.sqldb.Prepare("INSERT INTO repo_branches(repo_id, branch) VALUES ($1, $2)")
	if err != nil {
		return err
	}
...
func (db *DB) DeleteRepoBranch(repoID uint32, branch string) error {
	...
	// FIXME consider whether to move out into one-time-prepared statement
	stmt, err := db.sqldb.Prepare("DELETE FROM repo_branches WHERE repo_id = $1 AND branch = $2")
	if err != nil {
		return err
	}
	result, err = stmt.Exec(repoID, branch)
