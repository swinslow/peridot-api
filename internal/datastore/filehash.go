...
func (db *DB) GetFileHashByID(id uint64) (*FileHash, error) {
	var fh FileHash
	err := db.sqldb.QueryRow("SELECT id, hash_s256, hash_s1 FROM file_hashes WHERE id = $1", id).
...
func (db *DB) GetFileHashesByIDs(ids []uint64) ([]*FileHash, error) {
	rows, err := db.sqldb.Query("SELECT id, hash_s256, hash_s1 FROM file_hashes WHERE id IN ($1) ORDER BY id", ids)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) AddFileHash(sha256 string, sha1 string) (uint64, error) {
	stmt, err := db.sqldb.Prepare("INSERT INTO file_hashes(hash_s256, hash_s1) VALUES ($1, $2) RETURNING id")
	if err != nil {
		return 0, err
	}
...
func (db *DB) DeleteFileHash(id uint64) error {
	...
	stmt, err := db.sqldb.Prepare("DELETE FROM file_hashes WHERE id = $1")
	if err != nil {
		return err
	}
