...
func (db *DB) GetAllRepos() ([]*Repo, error) {
	rows, err := db.sqldb.Query("SELECT id, subproject_id, name, address FROM repos ORDER BY id")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) GetAllReposForSubprojectID(subprojectID uint32) ([]*Repo, error) {
	rows, err := db.sqldb.Query("SELECT id, subproject_id, name, address FROM repos WHERE subproject_id = $1 ORDER BY id", subprojectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) AddRepo(subprojectID uint32, name string, address string) (uint32, error) {
	...
	stmt, err := db.sqldb.Prepare("INSERT INTO repos(subproject_id, name, address) VALUES ($1, $2, $3) RETURNING id")
	if err != nil {
		return 0, err
	}
...
func (db *DB) UpdateRepo(id uint32, newName string, newAddress string) error {
	...
	if newName != "" && newAddress != "" {
		stmt, err := db.sqldb.Prepare("UPDATE repos SET name = $1, address = $2 WHERE id = $3")
		if err != nil {
			return err
		}
		...
		stmt, err := db.sqldb.Prepare("UPDATE repos SET name = $1 WHERE id = $2")
		if err != nil {
			return err
		}
		...
		stmt, err := db.sqldb.Prepare("UPDATE repos SET address = $1 WHERE id = $2")
		if err != nil {
			return err
		}
...
func (db *DB) UpdateRepoSubprojectID(id uint32, newSubprojectID uint32) error {
	...
	// FIXME consider whether to move out into one-time-prepared statement
	stmt, err := db.sqldb.Prepare("UPDATE repos SET subproject_id = $1 WHERE id = $2")
	if err != nil {
		return err
	}
...
func (db *DB) DeleteRepo(id uint32) error {
	...
	// FIXME consider whether to move out into one-time-prepared statement
	stmt, err := db.sqldb.Prepare("DELETE FROM repos WHERE id = $1")
	if err != nil {
		return err
	}
